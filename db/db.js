...
pool.getConnection((err, connection) => {
  ...
  connection.query(
    `CREATE TABLE IF NOT EXISTS tempandhums
          (temperature FLOAT(4,2) NOT NULL, humidity FLOAT(4,2) NOT NULL, time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, worker_name TEXT, state INT, workers_assessment TEXT)`, (err) => {
      if (err) throw err
    })
  connection.release()
})
...
class Tempandhums {
  ...
  static create (tah, callback) {
    ...
    pool.getConnection((err, connection) => {
      if (err) throw err
      connection.query(sql, [greeting.recipient, greeting.message], (err, results, fields) => {
        callback(err, results)
        connection.release()
      })
    })
  ...
  static insert (tah) {
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql, [tah.temperature, tah.humidity, worker.worker_name, worker.state, worker.workers_assessment], (err, results, fields) => {
        if (err) throw err
        connection.release()
      })
    })
