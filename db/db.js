...
pool.getConnection((err, connection) => {
  ...
  connection.query(
    `CREATE TABLE IF NOT EXISTS tempandhums
      (temperature FLOAT(4,2) NOT NULL, humidity FLOAT(4,2) NOT NULL, time TIMESTAMP DEFAULT CURRENT_TIMESTAMP PRIMARY KEY, worker_name TEXT, state INTEGER, workers_assessment TEXT)`, (err) => {
      if (err) throw err
    })
  connection.release()
})
...
class Tempandhums {
  ...
  static insert(tah, callback) {
    ...
    pool.getConnection((err, connection) => {
      ...
      const sql = 'INSERT INTO tempandhums(temperature, humidity, worker_name, workers_assessment) VALUES (?, ?, ?, ?)'
      connection.query(sql, [tah.temperature, tah.humidity, tah.worker_name, tah.workers_assessment], (err, results, fields) => {
        if (err) throw err
        connection.release()
      })
